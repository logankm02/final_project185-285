...
# Build Part 1 Submissions:

All commands run from the `final_project_llm_rl` folder. Each step generates a JSONL on the Modal volume, downloads it locally, and places it where the autograder expects it.

## Policy Submissions (DPO, IPO, AOT, GRPO, DrGRPO, GSPO)

### Build on Modal

Run one command per algorithm. Each writes to `/vol/submissions/<algo>.jsonl`.

**DPO** (final checkpoint: step\_000890):
```bash
uv run modal run scripts/modal_train.py::build_policy_submission_remote -- \
  --model_name Qwen/Qwen2.5-1.5B-Instruct \
  --adapter_path /vol/runs/wildchat_min4_judged_5k_dpo_beta01_v1/checkpoints/step_000890/adapter \
...
**IPO** (final checkpoint: step\_000890):
```bash
uv run modal run scripts/modal_train.py::build_policy_submission_remote -- \
  --model_name Qwen/Qwen2.5-1.5B-Instruct \
  --adapter_path /vol/runs/wildchat_min4_judged_5k_ipo_v1/checkpoints/step_000890/adapter \
  --prompts_jsonl /root/project/public_eval/public_test_gen_prompts_128.jsonl \
...
**AOT** (final checkpoint: step\_000890):
```bash
uv run modal run scripts/modal_train.py::build_policy_submission_remote -- \
  --model_name Qwen/Qwen2.5-1.5B-Instruct \
  --adapter_path /vol/runs/wildchat_min4_judged_5k_aot_beta02_v1/checkpoints/step_000890/adapter \
  --prompts_jsonl /root/project/public_eval/public_test_gen_prompts_128.jsonl \
...
**GRPO** (final checkpoint: step\_000025):
```bash
uv run modal run scripts/modal_train.py::build_policy_submission_remote -- \
  --model_name Qwen/Qwen2.5-1.5B-Instruct \
...
**DrGRPO** (final checkpoint: step\_000025):
```bash
uv run modal run scripts/modal_train.py::build_policy_submission_remote -- \
  --model_name Qwen/Qwen2.5-1.5B-Instruct \
  --adapter_path /vol/runs/wildchat_min4_judged_5k_drgrpo_rm445_v1/checkpoints/step_000025/adapter \
  --prompts_jsonl /root/project/public_eval/public_test_gen_prompts_128.jsonl \
...
**GSPO** (final checkpoint: step\_000025):
```bash
uv run modal run scripts/modal_train.py::build_policy_submission_remote -- \
  --model_name Qwen/Qwen2.5-1.5B-Instruct \
  --adapter_path /vol/runs/wildchat_min4_judged_5k_gspo_rm445_v1/checkpoints/step_000025/adapter \
  --prompts_jsonl /root/project/public_eval/public_test_gen_prompts_128.jsonl \
...
### Download policy submissions locally

```bash
mkdir -p llm_rl_final_proj_public_submission/policy_generations

for algo in dpo ipo aot grpo drgrpo gspo; do
  uv run modal volume get llm-rl-final-project-volume \
    /submissions/${algo}.jsonl \
    llm_rl_final_proj_public_submission/policy_generations/${algo}.jsonl
done
```

## Reward Model Submission

### Build on Modal (final checkpoint: step\_000445)
...
uv run modal volume get llm-rl-final-project-volume \
  ...
  llm_rl_final_proj_public_submission/reward_model/public_test_pref_scores.jsonl
```
...
# Test Part 2 Offline Submission:

The offline Part 2 methods (AOT-weighted, APO-Zero, APO-Down) all fall short of the 0.82 win-rate threshold required by the autograder. The steps below show how to build and score the best offline candidate so you can observe the failure.

## Build offline\_best.jsonl on Modal

The best offline checkpoint is AOT-weighted v2 step\_000890. Build it on Modal:

```bash
uv run modal run scripts/modal_train.py::build_policy_submission_remote -- \
  --model_name Qwen/Qwen2.5-1.5B-Instruct \
  --adapter_path /vol/runs/wildchat_min4_judged_5k_aot_weighted_v2/checkpoints/step_000890/adapter \
  --prompts_jsonl /root/project/public_eval/public_test_gen_prompts_128.jsonl \
...
## Download and set as active offline submission
